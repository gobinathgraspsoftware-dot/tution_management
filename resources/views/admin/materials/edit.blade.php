...
<form action="{{ route('admin.materials.update', $material) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card mb-4">
                <div class="card-header"><i class="fas fa-file-alt me-2"></i> Material Information</div>
                <div class="card-body">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $material->description) }}</textarea>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="grade_level_id" id="gradeLevelSelect" class="form-select @error('grade_level_id') is-invalid @enderror" required>
                                ...
                                @foreach($gradeLevels as $gl)
                                    <option value="{{ $gl->id }}" {{ old('grade_level_id', $material->grade_level_id) == $gl->id ? 'selected' : '' }}>{{ $gl->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="class_id" id="classSelect" class="form-select @error('class_id') is-invalid @enderror" required>
                                ...
                                @foreach($classes as $class)
                                    <option value="{{ $class->id }}" {{ old('class_id', $material->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }} - {{ $class->subject->name ?? 'N/A' }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="teacher_id" class="form-select @error('teacher_id') is-invalid @enderror" required>
                                ...
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ old('teacher_id', $material->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->user->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="alert alert-info mb-2">
                            <i class="fas fa-file me-2"></i><strong>{{ $material->title }}.{{ $material->file_type }}</strong>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" name="file" class="form-control @error('file') is-invalid @enderror" accept=".pdf,.doc,.docx,.ppt,.pptx,.mp4,.avi,.mov">
                        <small class="form-text text-muted">Replacing the file will reset approval status.</small>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select name="access_type" class="form-select @error('access_type') is-invalid @enderror" required>
                            <option value="view_only" {{ old('access_type', $material->access_type) == 'view_only' ? 'selected' : '' }}>View Only</option>
                            <option value="downloadable" {{ old('access_type', $material->access_type) == 'downloadable' ? 'selected' : '' }}>Downloadable</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="publish_date" class="form-control @error('publish_date') is-invalid @enderror" value="{{ old('publish_date', $material->publish_date?->format('Y-m-d')) }}">
                        @error('publish_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                </div>
            </div>
            <div class="card mb-4">
                <div class="card-header bg-info text-white"><i class="fas fa-info-circle me-2"></i> Current Status</div>
                <div class="card-body">
                    <div class="mb-2">
                        <small class="text-muted">Approval:</small>
                        <p class="mb-0">@if($material->is_approved)<span class="badge bg-success">Approved</span>@else<span class="badge bg-warning">Pending</span>@endif</p>
                    </div>
                    @if($material->approved_by)<div class="mb-2"><small class="text-muted">Approved By:</small><p class="mb-0">{{ $material->approvedBy->name ?? 'N/A' }}</p></div>@endif
                    <div><small class="text-muted">Uploaded:</small><p class="mb-0">{{ $material->created_at->format('d M Y') }}</p></div>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    <button type="submit" class="btn btn-primary w-100 mb-2"><i class="fas fa-save me-1"></i> Update Material</button>
                    <a href="{{ route('admin.materials.show', $material) }}" class="btn btn-outline-secondary w-100"><i class="fas fa-times me-1"></i> Cancel</a>
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('#gradeLevelSelect').on('change', function() {
        ...
        $.get('{{ route("admin.materials.get-subjects-by-grade-level") }}', {grade_level_id: id}, function(data) {
            ...
            data.forEach(s => opts += `<option value="${s.id}" ${curSub==s.id?'selected':''}>${s.name}</option>`);
    ...
    $('#subjectSelect').on('change', function() {
        ...
        $.get('{{ route("admin.materials.get-classes-by-filters") }}', {grade_level_id: glid, subject_id: sid}, function(data) {
            ...
            data.forEach(c => opts += `<option value="${c.id}" ${curCls==c.id?'selected':''}>${c.name} (${c.teacher_name})</option>`);
            $('#classSelect').html(opts).prop('disabled', false);
            if (curCls) $('#classSelect').val(curCls);
        });
    });

    if (curGL) $('#gradeLevelSelect').trigger('change');
});
</script>
@endpush
